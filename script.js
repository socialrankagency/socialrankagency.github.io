...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // header — cursor-reactive background glow + sliding "magnetic" pill behind nav links
  const headerEl = document.querySelector('header.site');
  const navLinksEl = document.querySelector('nav.links');
  const navPillEl = document.querySelector('.nav-pill');
  if (headerEl && !window.matchMedia('(prefers-reduced-motion: reduce)').matches && window.matchMedia('(pointer: fine)').matches) {
    headerEl.addEventListener('mousemove', (e) => {
      ...
      headerEl.style.setProperty('--hx', hx + '%');
      headerEl.style.setProperty('--hy', hy + '%');
    });
  }
  if (navLinksEl && navPillEl) {
    const links = navLinksEl.querySelectorAll('a');
    links.forEach((link) => {
      link.addEventListener('mouseenter', () => {
        navPillEl.style.width = link.offsetWidth + 'px';
        navPillEl.style.transform = `translateX(${link.offsetLeft}px)`;
      });
    });
  ...
  // hero "Live Rankings" chart — plays its draw-on animation only once the
  // card is actually visible, instead of on a fixed page-load timer. On
  // mobile the card sits below the hero text, so a load-timer version would
  // already be finished by the time someone scrolls down to see it.
      ...
      const rankObserver = new IntersectionObserver((entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            rankCard.classList.add('chart-play');
            rankObserver.unobserve(entry.target);
          }
        });
      }, { threshold: 0.3 });
      rankObserver.observe(rankCard);
    } else {
      rankCard.classList.add('chart-play');
    }
  }
  ...
  // back-to-top button — visible immediately on load
  ...
  // floating WhatsApp + call buttons — visible immediately on load
  const fabButtons = document.querySelectorAll('.fab');
  if (fabButtons.length) {
    fabButtons.forEach(btn => btn.classList.add('show'));
  }

  // cursor-reactive spotlight glow behind the banner text (desktop, fine pointer only)
  // applies to the homepage hero, every inner-page banner (about/contact/services/404),
  // the "How We Build" dev-section, and the footer CTA — i.e. every dark feature
  // section across the site
  const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  const finePointer = window.matchMedia('(pointer: fine)').matches;
  const bannerEls = document.querySelectorAll('.hero, .page-hero.page-hero--photo, .dev-section, .footer-cta');
  if (bannerEls.length && !reduceMotion && finePointer) {
    bannerEls.forEach((bannerEl) => {
      bannerEl.addEventListener('mousemove', (e) => {
        const rect = bannerEl.getBoundingClientRect();
        const mx = ((e.clientX - rect.left) / rect.width) * 100;
        const my = ((e.clientY - rect.top) / rect.height) * 100;
        bannerEl.style.setProperty('--mx', mx + '%');
        bannerEl.style.setProperty('--my', my + '%');
      });
    });
  }
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      ...
      const phoneCode = form.querySelector('#phone-code') ? form.querySelector('#phone-code').value : '';
      const phoneNumber = form.querySelector('#phone') ? form.querySelector('#phone').value.trim() : '';
