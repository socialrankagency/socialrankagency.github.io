...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // keep the header sticky everywhere on the page, but release it right as
  // the footer scrolls into view so it never sits on top of / hides the footer
  ...
  if (siteHeader && siteFooter && 'IntersectionObserver' in window) {
    const footerObserver = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        siteHeader.classList.toggle('header-unstick', entry.isIntersecting);
      });
