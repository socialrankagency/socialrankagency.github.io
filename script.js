...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // floating WhatsApp + call buttons — stay hidden while the hero
  // (with its stats row) is still on screen, so they never overlap it.
  // They fade in once the user has scrolled past it, on mobile and desktop alike.
  const fabButtons = document.querySelectorAll('.fab');
  if (fabButtons.length) {
    const heroEl = document.querySelector('.hero, .page-hero');
    const revealFabs = () => {
      const threshold = heroEl ? heroEl.getBoundingClientRect().height * 0.6 : 400;
      const shouldShow = window.scrollY > threshold;
      fabButtons.forEach(btn => btn.classList.toggle('show', shouldShow));
    };
    window.addEventListener('scroll', revealFabs);
    revealFabs();
  }
